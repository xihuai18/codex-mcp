...
```
参数：
├── action: "poll" | "respond_approval" | "respond_user_input"
├── sessionId: string
│
│ # poll 参数
├── cursor?: number          # 事件偏移量，默认 0
├── maxEvents?: number       # 最大事件数，默认 200
│
│ # respond_approval 参数
...
3. MCP 客户端响应：
   - 通过 codex_check(action="respond_approval") 发送决策
...
服务端在 `codex_check` 返回值中包含 `pollInterval` 字段，客户端应使用该值作为下次轮询间隔：

```
status = "waiting_approval" → pollInterval: 1000ms（需要快速响应审批）
status = "running"          → pollInterval: 3000ms（正常轮询）
status = "idle"/"error"/"cancelled" → pollInterval: undefined（终态，无需继续轮询）
```

说明：`running` 状态下长时间（30-60+ 秒）无新事件在模型推理阶段是可能的，不应直接判定失败。
...
2. 循环:
   ...
   c. 检查 actions 中是否有待审批项
      - 有: 展示给用户，收集决策，调用 codex_check({ action: "respond_approval", ... })
   d. 检查 status:
